...
ElseIf(($pyver -like "*Miniconda*" -Or $pyver -like "*Anaconda*") -And ($pyver -like "*2.*")) {
    ...
    If( $args[0] -eq "-y" -Or $args[0] -eq "-Y" -Or $args[0] -eq "yes" ) { install_python $url_python $true seaborn, pandas, ipython, numpy, matplotlib, snakemake, graphviz, spyder > $null}
    Else{
        $CONFIRM = Read-Host -Prompt "Do you want install a new version of Python, snakemake and other dependecies? [y/n]"
        If($CONFIRM -eq 'N' -Or $CONFIRM -eq 'n') { Write-Host "Abort" -ForegroundColor Red }
        Else { install_python $url_python $true seaborn, pandas, ipython, numpy, matplotlib, snakemake, graphviz, spyder > $null}
    }
}
Else{
    Write-Host "NOT FOUND" -ForegroundColor Red
...
If( $gcc -eq $null ){ # g++ not found
    Write-Host "NOT FOUND" -ForegroundColor Red
    If( $args[0] -eq "-y" -Or $args[0] -eq "-Y" -Or $args[0] -eq "yes" ){ install_g++ $url_gcc "." $true  > $null}
    Else{
        $CONFIRM = Read-Host -Prompt "Do you want install it? [y/n]"
        If($CONFIRM -eq 'N' -Or $CONFIRM -eq 'n') { Write-Host "Abort" -ForegroundColor Red }
        Else{ install_g++ $url_gcc "." $true > $null }
    }
}
...
If( $choco -eq $null ){
    ...
    Else{
        $CONFIRM = Read-Host -Prompt "Do you want install it? [y/n]"
        If($CONFIRM -eq 'N' -Or $CONFIRM -eq 'n') { Write-Host "Abort" -ForegroundColor Red}
        Else{ install_choco $url_choco > $null}
    }
}
Else { Write-Host "FOUND" -ForegroundColor Green}
...
If( $sub -eq $false){ # not installed
    ...
    If( $args[0] -eq "-y" -Or $args[0] -eq "-Y" -Or $args[0] -eq "yes" ){ install_sublime $url_sublime "." $true  > $null}
    Else{
        $CONFIRM = Read-Host -Prompt "Do you want install it? [y/n]"
        If($CONFIRM -eq 'N' -Or $CONFIRM -eq 'n') { Write-Host "Abort" -ForegroundColor Red}
        Else{ install_sublime $url_sublime "." $true  > $null}
    }
}
Else{ Write-Host "FOUND" -ForegroundColor Green}
...
If( -Not (Get-Command ninja -ErrorAction SilentlyContinue) ){ # ninja not installed
    Write-Host "NOT FOUND" -ForegroundColor Red
    If( $args[0] -eq "-y" -Or $args[0] -eq "-Y" -Or $args[0] -eq "yes" ){ install_ninja $url_ninja "." $true  > $null}
    Else{
        $CONFIRM = Read-Host -Prompt "Do you want install it? [y/n]"
        If($CONFIRM -eq 'N' -Or $CONFIRM -eq 'n') { Write-Host "Abort" -ForegroundColor Red}
        Else{ install_ninja $url_ninja "." $true  > $null}
    }
}
Else{ Write-Host "FOUND" -ForegroundColor Green}
