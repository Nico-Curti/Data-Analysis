...
template<typename T> inline void QDA<T>::train(const Patterns<T> &pattern, hyperparams<QDA> &p)
{
...
template<typename T> inline void QDA<T>::cv_train(const Patterns<T> &pattern, const std::vector<int> &idx, hyperparams<QDA> &p)
{
	this->cv_train(pattern, idx, p.prior);
}

template<typename T> void QDA<T>::cv_train(const Patterns<T> &pattern, const std::vector<int> &idx, T *prior)
{
	if(this->n_class == 0)
	{
		...
		this->n_class = (int)lbl.size();
		this->n_label = new int[this->n_class];
		for(const auto &u : lbl) this->n_label[u] = std::count(pattern.output, pattern.output + idx.size(), u);
	}
	if(prior != nullptr)
	{
		this->prior = new T[this->n_class];
...
template<typename T> int* QDA<T>::cv_test(const Patterns<T> &pattern, const std::vector<int> &idx)
{
	...
	for(int i = 0; i < (int)idx.size(); ++i)
	{
		...
		for(int c = 0; c < this->n_class; ++c)
		{
			...
			for(int j = 0; j < this->N; ++j)
			{
				m1 = pattern.input[idx[i]][j] - this->mean[c][j] / this->n_label[c];
				for (int k = 0; k < this->N; ++k)
					discr += (pattern.input[idx[i]][k] - this->mean[c][k] / this->n_label[c]) * this->cov_inv[c][j][k] * m1;
			}
			discr = -(T).5 * discr -(T).5 * this->det[c];
			discr += (this->prior != nullptr) ? log(this->prior[c]) : (T)0.;
			if(max < discr)
...
template<typename T> int QDA<T>::predict(T *input)
{
	int score;
	T max = -std::numeric_limits<T>::infinity(), discr, m1;
	for(int c = 0; c < this->n_class; ++c)
	{
		discr = (T)0.;
		for(int j = 0; j < this->N; ++j)
		{
			m1 = input[j] - this->mean[c][j] / this->n_label[c];
			for (int k = 0; k < this->N; ++k)
				discr += (input[k] - this->mean[c][k] / this->n_label[c]) * this->cov_inv[c][j][k] * m1;
		}
		discr = -(T).5 * discr -(T).5 * this->det[c];
		discr += (this->prior != nullptr) ? log(this->prior[c]) : (T)0.;
		if(max < discr)
